...
Player.goTo = function(direction) {
    ...
    if (direction === 'next') {
        ...
        if (this.proxy.pointer >= this.slidesMap.length) {
            ...
            alignOffsetAdjustment = this.getCellAlignOffsetAdjustment(0);

            this.proxy.pointer = 0;
        ...
        if(!this.proxy.isSettled && this.proxy.isWrapProcessOngoing) {
            this.proxy.isAugmentedOffset = true;
            denormalizedOffset += this.proxy.totalSlidesWidth;
        }
...
Player.slide = function(offset, startingXCssTranlation) {
    ...
    function step(timestamp) {
        ...
        if (_proxy.targetOffset > 0) {
            ...
            if (_proxy.isWrapProcessOngoing && !_proxy.isFirstElTranslated && Math.abs(progressDelta) >= (_slidesMap[lastCellIndex].width / 2)) {
                ...
                _proxy.isWrapProcessOngoing = false;
            ...
            if (_proxy.isWrapProcessOngoing && _proxy.isFirstElTranslated && Math.abs(progressDelta) >= (_slidesMap[0].width / 2)) {

                // Update the pointer to the first cell
                //_proxy.pointer = 0;
                if(_proxy.isAugmentedOffset) {
                    console.log('remove augmented offset');
                    console.log(_proxy.targetOffset);
                    _proxy.targetOffset += _proxy.totalSlidesWidth;
                    console.log(_proxy.targetOffset);
                    _proxy.isAugmentedOffset = false;
                    /*cAF(animation);
                    return false;*/
                }
    ...
    this.proxy.animation = animation = rAF(step);
};
